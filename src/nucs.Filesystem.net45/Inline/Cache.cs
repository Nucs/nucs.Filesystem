...
namespace nucs.SystemCore {
    internal class Cache<T> : IDisposable {
        ...
        internal static Cache<T> Minute(Func<T> update, bool constantly_update) {
            return new Cache<T>(update, 60*1000, constantly_update);
        }

        internal static Cache<T> Hour(Func<T> update, bool constantly_update) {
            return new Cache<T>(update, 60*60*1000, constantly_update);
        }

        internal static Cache<T> FiveSeconds(Func<T> update, bool constantly_update) {
            return new Cache<T>(update, 5*1000, constantly_update);
        }

        internal static Cache<T> Seconds(Func<T> update, uint seconds, bool constantly_update) {
            return new Cache<T>(update, seconds * 1000, constantly_update);
        }
        internal static Cache<T> Minutes(Func<T> update, uint minutes, bool constantly_update) {
            return new Cache<T>(update, minutes *60* 1000, constantly_update);
        }

        internal static Cache<T> Hours(Func<T> update, uint hours, bool constantly_update) {
        ...
        /// </summary>
        internal readonly bool ConstantlyUpdate;
        ...
        /// </summary>
        internal readonly Func<T> Update;
        ...
        ///     access.
        /// </param>
        internal Cache(Func<T> update, uint cachetimeout, bool constantly_update) : this(default(T), update, cachetimeout, constantly_update) {}
        ...
        ///     access.
        /// </param>
        internal Cache(T value, Func<T> update, uint cachetimeout, bool constantly_update) {
        ...
        private class CountdownTimer
        {
            ...
            internal CountdownTimer(int interval, bool start = false)
            {
                _interval = interval;
                enabled = interval > 0;
