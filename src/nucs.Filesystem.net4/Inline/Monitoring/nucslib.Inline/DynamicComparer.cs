...
internal delegate bool CompareEqualityHandler<in T>(T x, T y);

internal delegate int HashcodeHandler<in T>(T x);
...
public class DynamicEqualityComparer<T> : EqualityComparer<T> {
    ...
    internal DynamicEqualityComparer(CompareEqualityHandler<T> comparer, HashcodeHandler<T> hashcalc) {
